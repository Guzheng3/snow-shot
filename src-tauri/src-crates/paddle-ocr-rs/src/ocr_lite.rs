...
impl OcrLite {
    ...
    /// 通过外部字典文件设置识别模型的字符表（解决 PP-OCRv6 不内嵌 character 元数据的问题）。
    /// 需在 init_models_from_memory_custom 之后调用。
    pub fn init_rec_keys_from_path(&mut self, path: &str) -> Result<(), OcrError> {
        self.crnn_net.init_keys_from_path(path)
    }
